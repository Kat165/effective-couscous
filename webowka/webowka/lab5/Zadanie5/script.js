...
//1
var PATTERN = /r/,
    filtered = names.filter(function (str) { return PATTERN.test(str); });

let imiona = document.getElementById('imiona');

function setRNames(){
    imiona.innerHTML = filtered;
}

//2
let numbs = document.getElementById("tablica")

const result = numbers.filter(n => n < 9);

const result2 = numbers.filter(n => n < 6);

function setNumbers(){
    numbs.innerHTML = result;
}